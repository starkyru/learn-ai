...
Here you learn everything that happens *around* that call in a real application:
conversation history, streaming UX, cost accounting, structured data extraction,
tool calling, and making your code resilient to flaky APIs.
...
Server-Sent Events (SSE). The SDK converts SSE into a Python iterator / TS async iterable.
**Time-to-first-token (TTFT)** is the latency the user actually *perceives*;
total generation time matters for throughput but TTFT matters for feel.

### Tokens and cost

LLMs process *tokens*, not words. A token is roughly 3-4 characters of English text.
`tiktoken` (used for GPT models) lets you count tokens client-side.
...
Asking a model to return JSON requires:
...
This is **intentional**: the abstraction leaks here to teach you *where* it leaks and why.
...
- [tiktoken repo](https://github.com/openai/tiktoken) — how BPE tokenization works
