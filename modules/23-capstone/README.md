...
| Agent loop: the LLM can request more context, call a tool, or answer | 06, 17 |
| At least one tool (e.g. search, calculator, date lookup, SQL query) | 02, 06, 12 |
...
| M2 — Hybrid retrieve + rerank | Add BM25 alongside dense retrieval, fuse with RRF, and optionally add a cross-encoder reranker. Compare before/after. | Hybrid beats dense-only MRR@5 on your 5 questions. |
...
- Add red-team tests (module 20) to the CI eval gate.
...
**Required capabilities:** planning, web/RSS retrieval (real or mocked), multi-step
...
LLM (modules 09, 11), embedding captions/text for retrieval (module 04), a RAG
...
**Stretch goals:** CLIP-based zero-shot image search (module 09), audio
...
- `modules/06-agents/` — ReAct loop, native tool calling, multi-agent
...
- `modules/12-text-to-sql/` — NL→SQL if your corpus includes structured data
...
- `modules/22-ai-product-ux/` — streaming SSE, citations UI, feedback capture, approval flow
