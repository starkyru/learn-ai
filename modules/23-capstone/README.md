...
| M6 — Accountable release                                                                  | Containerise and deploy a revision through staging following 07b's [`RUNBOOK.md`](../07b-delivery-operations/RUNBOOK.md); apply auth/tenant filtering, retention decisions, gold-evidence evals, and deterministic agent-safety tests; perform a rollback exercise (modules 07b, 20b, 21b). | A **staging revision is deployed and recorded** (image digest + config revision); a **cross-tenant boundary test passes**; **migration-from-empty and a rollback exercise are evidenced** (per the 07b RUNBOOK); release runbook, data map, model/data card, and rollback threshold are reviewed; held-out evidence and agent-policy suite pass. |
...
- [ ] If deployed (M6): a staging revision is recorded (image digest + config
      revision), a cross-tenant boundary test passes, and migration-from-empty +
      rollback evidence is attached — following the 07b
